            ...
            <div>
                <div className="flex flex-wrap justify-center mb-8 space-x-4">
                    {visibleCategories.map((category, index) => (
                        <span
                            key={index}
                            className={`cursor-pointer transition duration-300 text-base font-semibold hover:text-customRed ${selectedCategory === category ? 'text-blue-600 border-b-2 border-blue-600' : 'text-subText'}`}
                    ...
                    {dropdownCategories.length > 0 && (
                        <div className="relative">
                            ...
                            {showDropdown && (
                                <div
                                    className="absolute z-10 bg-white border rounded shadow-lg mt-2">
                                    {dropdownCategories.map((category, index) => (
                                        <span
                                            key={index}
                                            className={`block px-4 py-2 cursor-pointer hover:bg-customRed transition duration-300 text-base font-semibold ${selectedCategory === category ? 'text-blue-600' : 'text-subText'}`}
                                            onClick={() => {
