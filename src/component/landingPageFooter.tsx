      ...
      <div className=" basis-[90%] flex justify- items-center h-full">
        <div className="flex gap-8">
          {pagePath.map((data, index) => {
            return (
              <Link style={{ textDecoration: 'none' }} key={data.id} href={data.linkOfTheSection}>
                <motion.div
                  ...
                  key={data.id}
                  className={`${
                    TabIndex === index ? 'border-b-2 font-semibold border-white ' : ''
                  }`}
                >
                  <p className="text-white  hover:cursor-pointer">{data.namaOfSection}</p>
                </motion.div>
              </Link>
            );
          })}
