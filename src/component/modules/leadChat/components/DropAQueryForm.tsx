  ...
  return (
    <div>
      ...
      <div className="flex px-5 flex-col">
        ...
        <label className={`text-red-600   text-xs py-1 ${errors.phone ? 'visible' : 'invisible'}`}>
      ...
      <div className="flex px-5 flex-col">
        ...
        <label className={`text-red-600   text-xs py-1 ${errors.email ? 'visible' : 'invisible'}`}>
      ...
      <div>
        ...
        <span className="text-xs">
          I authorise Sandip University and its representatives to Call, SMS, Email or WhatsApp me
          about its programmes and offers. This consent overrides any registration for DNC / NDNC.
        </span>
      </div>
      <div className="w-full flex justify-center  mb-3 items-center">
        {messageSent ? (
          <p className="text-buttonBlue  text-md font-semibold mb-4 ">{`Your message is sent.`}</p>
        ) : (
          <button
            onClick={submit}
            disabled={!isButtonVisble}
            className={`text-white  transition duration-500  rounded-md hover:scale-105 ease-out  placeholder:text-sm bg-buttonBlue hover:bg-buttonBlue py-2 focus:ring-1 focus:outline-none focus:ring-buttonBlue font-medium  text-sm px-4   ${
