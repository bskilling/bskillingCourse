...
const CourseDetails = () => {
    ...
    const fetchCourseDetails = async (id: string) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_TRAINING_BASE_URL}api/v1/get-course/${id}`);
            if (!response.ok) {
        ...
        <div className='flex flex-col min-h-screen bg-gray'>
            ...
            <div className="relative w-full h-[200px] md:h-[300px] lg:h-[400px] flex items-center justify-center bg-gradient-background">
                ...
                <div className='w-full flex flex-col md:flex-row justify-between space-y-4 md:space-y-0'>
                    {/* Left Side Content */}
                    <div className="w-2/3 z-10 p-4 text-white text-center md:text-left px-16">
                        <h1 className="text-xl md:text-2xl lg:text-3xl font-bold tracking-wider mb-4">{courseDetails?.title}</h1>
                        <div className='flex flex-col md:flex-row items-center md:items-start space-y-2 md:space-y-0 md:space-x-4'>
                            <button className='bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors'>Best Seller</button>
                            <p className='text-sm md:text-base mt-[10px]'>{courseDetails?.ratings || '4.5 (11,234 Ratings)'}</p>
                        </div>
                        <div className='flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-4 mt-2'>
                            ...
                            <p>Enrollment End Date: <span className='font-bold'>{formatDate(batchTime) || '05/2024'}</span></p>
                        </div>
                        <div className="relative">
                            <div>
                                <p
                                    className="cursor-pointer italic font-bold text-white hover:text-blue-800"
            ...
            {/* video section */}
            <div className='flex-grow px-4 md:px-16 mt-12 mb-2 flex flex-col md:flex-row gap-12'>
                <div className='flex-grow w-3/4'>
                    {courseDetails?.training_metadata?.preview_video && (
                        <div className="relative w-full" style={{ paddingTop: '30.25%' }}> {/* 16:9 Aspect Ratio */}
                            <iframe
