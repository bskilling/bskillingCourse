...
const CourseEnrollment: React.FC<CourseEnrollmentProps> = ({
  ...
  // Calculate 20% discount and original price
  const discountPercentage = 20;
  // Convert formatted price to number (remove commas and convert to number)
  const priceValue = parseInt(formattedPrice.replace(/,/g, ''));
  // Calculate original price (assuming formattedPrice is already discounted)
  const originalPrice = Math.round(priceValue / (1 - discountPercentage / 100));
  // Format original price with commas
  const formattedOriginalPrice = originalPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',');
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 py-4 px-6">
          <p className="text-white text-center text-sm font-medium">
            {isPaid
              ? `Limited Time Offer - ${discountPercentage}% OFF for a limited time!`
              ...
              {isPaid ? (
                <>
                  <div className="text-left ">
                    <p className="text-blue-600 text-4xl font-bold lavishly-yours-regular">
                      Starts{' '}
                    </p>
                  </div>
                  <div className="flex items-center justify-center gap-2 mt-1 -ml-9">
                    <p className="text-blue-600 text-4xl  font-bold lavishly-yours-regular">from</p>
                ...
                <div className="mt-1">
                  ...
                  <span className="text-4xl font-bold text-gray-900 block mt-1">FREE</span>
